...
const state = { cat: q.get('cat'), kind: q.get('kind'), cls: q.get('class'), release: q.get('release') === '1', q: q.get('q') ?? '', sort: ((['best', 'new', 'az'] as SortKey[]).includes(q.get('sort') as SortKey) ? q.get('sort') : 'best') as SortKey };
const focus = q.get('focus');
function sync(): void { const u = new URLSearchParams(); if (state.cat) u.set('cat', state.cat); if (state.kind) u.set('kind', state.kind); if (state.cls) u.set('class', state.cls); if (state.release) u.set('release', '1'); if (state.q) u.set('q', state.q); if (state.sort !== 'best') u.set('sort', state.sort); history.replaceState(null, '', u.toString() ? `?${u}` : location.pathname); }
...
function render(): void {
  ...
  const shown = sortEntries(entries.filter((e) => (!state.cat || e.category === state.cat) && (!state.kind || e.kind === state.kind) && (!state.cls || e.class === state.cls) && (!state.release || e.state === 'RELEASED' || e.state === 'VERIFYING') && (!s || (e.title + ' ' + (e.why ?? '') + ' ' + e.description + ' ' + e.slug + ' ' + (e.tags ?? []).join(' ')).toLowerCase().includes(s))), state.sort);
  const c = state.cat ? cats.get(state.cat) : null;
  title.textContent = c ? c.title : state.kind ? (KINDS[state.kind] ?? state.kind) : state.cls ? (KIND[state.cls] ?? state.cls) : state.release ? 'Just released' : 'All tools';
  blurb.textContent = c?.blurb ?? (state.kind === 'game' ? 'Games listed in the catalogue.' : state.release ? 'Verified releases and tools still in their first QA check.' : '');
